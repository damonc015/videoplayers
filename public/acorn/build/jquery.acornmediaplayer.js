...
(function ($) {
  $.fn.acornMediaPlayer = function (options) {
     ...
     */
    var volume = supports_local_storage
      ? localStorage.getItem("acornvolume")
      : 1;
    if (!volume) {
      volume = 1;
    }
    ...
    var acornPlayer = function () {
      ...
      var $wrapper = $(
        '<div class="acorn-player" role="application"></div>',
       ...
       */
      var fullscreenBtnMarkup = acorn.$self.is("video")
        ? '<button class="acorn-fullscreen-button" title="' +
       ...
       */
      var tooltipMarkup = options.tooltipsOn
        ? '<div class="acorn-tooltip"><div>'
        : "";
      ...
      var template =
        ...
        '<button class="acorn-play-button" title="' +
        text.playTitle +
        '" aria-controls="' +
        acorn.id +
        '">' +
        text.play +
        "</button>" +
        '<input type="range" class="acorn-seek-slider" title="' +
        text.seekTitle +
        '" value="0" min="0" max="150" step="0.1" aria-controls="' +
        acorn.id +
        '"/>' +
        '<span class="acorn-timer">00:00</span>' +
        '<div class="acorn-volume-box">' +
        '<button class="acorn-volume-button" title="' +
        text.mute +
        '" aria-controls="' +
        acorn.id +
        '">' +
        text.mute +
        "</button>" +
        '<input type="range" class="acorn-volume-slider" title="' +
        text.volumeTitle +
        '" value="1" min="0" max="1" step="0.05" aria-controls="' +
        acorn.id +
        '"/>' +
        "</div>" +
        fullscreenBtnMarkup +
        '<button class="acorn-caption-button" title="' +
        text.captionsTitle +
        '"  aria-controls="' +
        acorn.id +
        '">' +
        ...
        '<button class="acorn-transcript-button" title="' +
      ...
      var transcriptMarkup =
        '<div class="acorn-transcript" role="region" aria-live="assertive"></div>';
      ...
      var timeFormat = function (sec) {
        var m =
          Math.floor(sec / 60) < 10
            ...
            : Math.floor(sec / 60);
        var s =
          Math.floor(sec - m * 60) < 10
            ? "0" + Math.floor(sec - m * 60)
            : Math.floor(sec - m * 60);
        return m + ":" + s;
      };
      ...
      var updateSeek = function () {
        ...
        if (options.nativeSliders) {
          ...
          initSliderAccess(
            acorn.$seek.find(".ui-slider-handle"),
            sliderOptions,
          );
      ...
      var initVolume = function () {
        if (options.nativeSliders) {
          ...
          volumeSliderOptions.valuetext =
            volumeSliderOptions.value + " percent";
      ...
      var initCaptionSelector = function () {
        ...
        var setUpCaptionSelector = function () {
          ...
          var left =
            pos.left -
            (acorn.$captionSelector.outerWidth(true) -
              acorn.$captionBtn.outerWidth(true)) /
              2;
        ...
        $("input[name=" + captionRadioName + "]", acorn.$container).bind(
          "focus",
          showCaptionSelector,
        );
        $("input[name=" + captionRadioName + "]", acorn.$container).bind(
      ...
      var loadCaption = function (url) {
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            acorn.$captionBtn
              .addClass(captionBtnActiveClass)
              .removeClass(captionBtnLoadingClass);
          },
          error: function () {
      ...
      var initCaption = function () {
        ...
        if (acorn.$track.length > 1) {
          ...
          var captionList =
            '<ul><li><label><input type="radio" name="' +
            captionRadioName +
            '" checked="true" />None</label></li>';
          ...
          $("input[name=" + captionRadioName + "]", acorn.$container).change(
            changeCaption,
          );
          ...
          if (options.captionsOn) {
            ...
            $("input[name=" + captionRadioName + "]", acorn.$container).prop(
              "checked",
              false,
            );
            $(
              "input[name=" + captionRadioName + "]:eq(1)",
              acorn.$container,
            ).prop("checked", true);
          }
